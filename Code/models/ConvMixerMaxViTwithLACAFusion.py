...
def _make_block_input_shapes(
    ...
    """Util function to compute input shapes for each block."""
    shapes = []
    block_input_shape = input_size
    for i in range(n_blocks):
        shapes.append(block_input_shape)
        if downsample_depth_schedule[i]:
            # Full 3D downsampling
            block_input_shape = _get_conv_output_shape(block_input_shape, 3, 2, 1)
        else:
            # Only downsample H and W, preserve D
            block_input_shape = _get_conv_output_shape(
                block_input_shape, 
                kernel_size=(1, 3, 3), 
                stride=(1, 2, 2), 
                padding=(0, 1, 1)
...
class ConvMixerMaxVit(nn.Module):
    def __init__(
        self, 
        input_size:tuple[int,int,int],
        stem_channels:int,
        ...
        input_size = (input_size[0]//ConvMixer_patch_size[0], input_size[1]//ConvMixer_patch_size[1], input_size[2]//ConvMixer_patch_size[2])
        
        block_input_sizes = _make_block_input_shapes(input_size, len(block_channels),downsample_depth_schedule)
        ...
        self.stem = ConvMixer(stem_channels,ConvMixer_depth,ConvMixer_kernel_size,ConvMixer_patch_size)
...
class Model(nn.Module):
    def __init__(
        ...
        self.fusion = latentAlignmentCrossAttentionFusion(
            in_dim = 128,
            hidden_dim = LACA_hidden_dim,
            out_dim = LACA_out_dim,
        )
        self.mols_encoder = nn.Sequential(
            nn.Linear(2,mols_dim),
            nn.ReLU(),
            nn.BatchNorm1d(mols_dim)
        )
