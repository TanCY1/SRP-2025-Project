...
class Embed(nn.Module):
    def __init__(self,dim,depth,kernel_size,image_size,patch_size):
        super().__init__()
        ...
        self.patchedEmbed = ConvMixer(dim,depth,kernel_size,patch_size)
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        num_patches = math.prod(i//p for i,p in zip(image_size,patch_size))
        self.positionEmbed = nn.Parameter(torch.randn(1,num_patches+1,dim))
    def forward(self,x):
...
class VolumeEncoder(nn.Module):
    def __init__(self,image_size,hidden_dim,depth,kernel_size,patch_size,num_heads,feedforward_dim,num_blocks,use_cls):
        super().__init__()
        self.embed = Embed(hidden_dim,depth,kernel_size,image_size,patch_size)
...
class Model(nn.Module):
    def __init__(self,image_size,hidden_dim,depth,kernel_size,patch_size,num_heads,feedforward_dim,num_blocks,mol_dim,hidden_fusion_dim):
        super().__init__()
        self.preNac = VolumeEncoder(image_size,hidden_dim,depth,kernel_size,patch_size,num_heads,feedforward_dim,num_blocks,use_cls=True)
        self.postNac = VolumeEncoder(image_size,hidden_dim,depth,kernel_size,patch_size,num_heads,feedforward_dim,num_blocks,use_cls=True)
