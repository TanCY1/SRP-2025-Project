...
class Model(nn.Module):
    def __init__(self,dim,depth,kernel_size,patch_size,mol_dim,CA_feedforward_dim,hidden_fusion_dim):
        super().__init__()
        self.preNac = ConvMixer(dim,depth,kernel_size,patch_size)
        self.postNac = ConvMixer(dim,depth,kernel_size,patch_size)
        self.dropout = nn.Dropout()
        self.fusion = crossAttentionFusion(dim,CA_feedforward_dim,True)
        self.mols_encoder = nn.Sequential(
            nn.Linear(2,mol_dim),
            nn.ReLU(),
            nn.BatchNorm1d(mol_dim)
        )
        self.fc1 = nn.Linear(dim+mol_dim,hidden_fusion_dim)
